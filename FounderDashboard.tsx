...
import { 
  ...
  Filter, MoreVertical, Eye, UserCheck, UserX,
...
const FounderDashboard: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'users' | 'ideas' | 'transactions'>('overview');
    ...
    <div className="pt-24 pb-20 px-4 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        ...
        <div className="flex gap-2 bg-white/5 p-1 rounded-xl border border-white/10">
          {(['overview', 'users', 'ideas', 'transactions'] as const).map((tab) => (
            <button
              key={tab}
              ...
              <div className="h-80 w-full flex items-center justify-center">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      data={[
                        { name: 'Criadores', value: data.users.filter((u: any) => u.role === 'creator').length },
                        { name: 'Compradores', value: data.users.filter((u: any) => u.role === 'buyer').length },
                        { name: 'Staff', value: data.users.filter((u: any) => u.role === 'admin' || u.role === 'founder').length },
                      ]}
                      cx="50%"
                      cy="50%"
                      innerRadius={60}
