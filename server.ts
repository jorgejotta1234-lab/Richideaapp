...
async function startServer() {
  ...
  app.get("/api/auth/me", authenticateToken, (req: any, res) => {
    const user: any = db.prepare("SELECT id, name, email, role, country, wallet_balance, kyc_status, profile_image FROM users WHERE id = ?").get(req.user.id);
    res.json(user);
  });
  ...
  app.post("/api/users/kyc", authenticateToken, (req: any, res) => {
  ...
  app.post("/api/ideas", authenticateToken, (req: any, res) => {
  ...
  app.get("/api/ideas/:id", authenticateToken, (req: any, res) => {
    const idea: any = db.prepare("SELECT * FROM ideas WHERE id = ?").get(req.params.id);
    ...
    if (!nda && !isOwner && req.user.role !== 'admin') {
      // Return partial data
      const { description_encrypted, ...partial } = idea;
  ...
  // NDAs
  app.post("/api/ndas/sign", authenticateToken, (req: any, res) => {
  ...
  app.post("/api/transactions/buy", authenticateToken, (req: any, res) => {
    const { idea_id } = req.body;
    const idea: any = db.prepare("SELECT * FROM ideas WHERE id = ?").get(idea_id);
    if (!idea) return res.status(404).json({ error: "Idea not found" });
    
    const buyer: any = db.prepare("SELECT wallet_balance FROM users WHERE id = ?").get(req.user.id);
    if (buyer.wallet_balance < idea.price) {
      return res.status(400).json({ error: "Insufficient funds in wallet" });
    }

    const commission = idea.price * 0.10;
    const seller_amount = idea.price - commission;
  ...
  // Security & Anti-fraud
  app.post("/api/security/log", (req: any, res) => {
  ...
  app.get("/api/admin/security-logs", authenticateToken, (req: any, res) => {
    if (req.user.role !== 'admin' && req.user.role !== 'founder') return res.status(403).json({ error: "Unauthorized" });
  ...
  app.get("/api/founder/full-stats", authenticateToken, (req: any, res) => {
  ...
  // Chat
  app.get("/api/chat/active", authenticateToken, (req: any, res) => {
  ...
  app.get("/api/chat/:idea_id", authenticateToken, (req: any, res) => {
    const { idea_id } = req.params;
    
    // Check if user is creator or has signed NDA
    const idea: any = db.prepare("SELECT creator_id FROM ideas WHERE id = ?").get(idea_id);
    if (!idea) return res.status(404).json({ error: "Idea not found" });
  ...
  app.post("/api/chat/:idea_id", authenticateToken, (req: any, res) => {
    const { idea_id } = req.params;
